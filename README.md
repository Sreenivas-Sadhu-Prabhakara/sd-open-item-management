...
| POST | `/v1/open-item-management-plan/initiate` | Initiate |
| GET | `/v1/open-item-management-plan` | Retrieve (list) |
| GET | `/v1/open-item-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/open-item-management-plan/{crId}/update` | Update |
| PUT | `/v1/open-item-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/open-item-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-open-item-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-open-item-management ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-open-item-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
